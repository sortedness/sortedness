...
def wlossf4(predicted_ranks, expected_ranks, smoothness_ranking, lambd):
    n, o = predicted_ranks.shape
    mu = 0
    for pred, target in zip(predicted_ranks, expected_ranks):
        ...
        mu += surrogate_wtau2(pred, target, w_p, w_t, lambd)
    return mu / n


def wlossf5(predicted_ranks, expected_ranks, lambd):
    n, o = predicted_ranks.shape
    mu = 0
    for pred, target in zip(predicted_ranks, expected_ranks):
...
def wlossf6(predicted_D, expected_R, smoothness_ranking, lambd, ref, k, gamma, w):
    ...
    for pred_d, target_r in zip(predicted_D, expected_R):
        ...
        # mu += surrogate_wtau(pred_d, target_r, w_p, lambd)
        mu += surrogate_wtau2(pred_d, target_r, w_p, w_t, lambd)
        if ref:
...
def wlossf8(predicted_D, expected_R, smoothness_ranking, lambd, ref, k, gamma, w):
    n, o = predicted_D.shape
    ...
    for pred_d, target_r in zip(predicted_D, expected_R):
        ...
        surrtau = surrogate_wtau(a, b, w, lambd)  # todo: empates, diagonal, normalização p/ compatibilizar X com X_
        # surrtau_ = surrogate_wtau(a_, b_, w, lambd)
        # mu_local = (surrtau + surrtau_) / 2
        # mu_global = surrogate_tau(pred_d, target_r, lambd) / (o ** 2 - o) * 2
...
def wlossf10(predicted_D, expected_R, smoothness_ranking, lambd, ref, k, gamma, w):
    n, o = predicted_D.shape
    mu = tau = 0
    for pred_d, target_r in zip(predicted_D, expected_R):
        w = cau(soft_rank(pred_d.view(1, o), regularization_strength=smoothness_ranking).view(o), gamma=gamma)
        mu_local = surrogate_wtau(pred_d, target_r, w, lambd)
        mu_global = surrogate_tau(pred_d, target_r, lambd) / (o ** 2 - o) * 2
...
def wlossf11(predicted_D, expected_R, smoothness_ranking, lambd, ref, k, gamma, w):
    n, o = predicted_D.shape
    mu = tau = 0
    for pred_d, target_r in zip(predicted_D, expected_R):
        w = cau(soft_rank(pred_d.view(1, o), regularization_strength=smoothness_ranking).view(o), gamma=gamma)
...
def wlossf12(predicted_D, expected_R, smoothness_ranking, lambd, ref, k, gamma, w):
    n, o = predicted_D.shape
    mu = tau = 0
    for pred_d, target_r in zip(predicted_D, expected_R):
        a, idxs = topk(pred_d, k, largest=False)
        b = target_r[idxs]
        mu_local = surrogate_wtau(a, b, w[:k], lambd)
        mu += mu_local
...
def wlossf9(predicted_D, expected_R, smoothness_ranking, lambd, ref, k, gamma, w, wharmonic):
    n, o = predicted_D.shape
    mu = tau_local = tau_global = 0
    for pred_d, target_r in zip(predicted_D, expected_R):
        # pmn, tmn = torch.min(pred_d), torch.min(target_r)
        ...
        # b = target_r[idxs]
        # mu_local = surrogate_wtau(a, b, w[:k], lambd)
        mu_global = surrogate_wtau(pred_d, target_r, wharmonic, lambd) #/ (o ** 2 - o) * 2
        mu += mu_global
        # l = (mu_local + 1) / 2
        # g = (mu_global + 1) / 2
        # alpha = 0.5
