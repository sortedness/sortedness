...
from torch import manual_seed, from_numpy, sum
...
class M(Module):
    def __init__(self, n_inputs):
        ...
        m, n = 3, 2
        self.enc = Sequential(
            Linear(n_inputs, m), Tanh(),
            Linear(m, n), Tanh(),
            Linear(n, 1), Sigmoid()
        )
        self.asd = list(self.enc.parameters())

    def forward(self, X):
        return self.enc(X)

    def latent_weights(self):
        vs = list(self.state_dict().values())
        ls = vs[::2]
        bs = vs[1::2]
        for lay in ls:
            ws = []
            for neu in lay:
                values,idxs = abs(neu).sort()
                ws.append(values)

        ls = [ for neu in lay)) for lay in ls]
        bs = [list(sorted((abs(lay).sum().item(), abs(lay).sort()[0].tolist()) for lay in bs]
        pprint(ls)
        print()
        pprint(bs)
        # pprint([param[0] for param in self.asd])
        exit()
