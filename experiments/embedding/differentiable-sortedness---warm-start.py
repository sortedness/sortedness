...
if gpu:
    w = w.cuda()
loss, ref = f(D, R, smoothness_ranking[0], lambd[0], ref=True, k=k, gamma=gamma, w=w)
ax[0].cla()
...
print(f"{0:09d}:\toptimized sur: {loss:.8f}\tresulting wtau: {ref}\t\t{smoothness_ranking[0]:.6f}\t{lambd[0]:.6f}")
...
def animate(i):
    ...
    for idx in loader[0]:
        ...
        loss, ref_ = f(D_batch, expected_ranking_batch, smoothness_ranking[0], lambd[0], ref=i % update == 0, k=k, gamma=gamma, w=w)
        if ref_ != 0:
            ref = ref_
        (-loss).backward()
        optimizer.step()
    ...
    if i % update == 0:
        ...
        lambd[0] *= 1 - decay
        print(f"{i:09d}:\toptimized sur: {loss:.8f}\tresulting wtau: {ref}\t\t{smoothness_ranking[0]:.6f}\t{lambd[0]:.6f}")
